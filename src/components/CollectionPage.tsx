...
export const CollectionPage: React.FC<CollectionPageProps> = ({
  ...
  return (
    <div className="bg-[#FAFAFA] min-h-screen py-8 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Breadcrumb Navigation */}
        <nav className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-gray-500 mb-4 font-light">
          <button 
            type="button" 
        ...
        <div className="bg-white p-6 sm:p-8 border border-gray-200/80 mb-8 text-center sm:text-left flex flex-col sm:flex-row justify-between items-center gap-6">
          <div className="max-w-2xl">
            <span className="text-[9px] font-light uppercase tracking-[0.25em] text-gray-500 block mb-1">
              Curated Essentials
            </span>
            <h1 className="text-2xl sm:text-3xl text-[#111111] font-light tracking-tight">
              {activeHeader.title}
            </h1>
            <p className="text-xs text-gray-500 mt-1.5 leading-relaxed font-light tracking-wide">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {(['all', 'shirts', 'pyjamas', 'boxers', 'lounge-pants'] as CategoryType[]).map((cat) => (
              <button
                ...
                className={`px-3.5 py-1.5 text-[10px] font-normal uppercase tracking-[0.18em] transition-all border ${
                  category === cat
                    ? 'bg-[#111111] text-white border-black shadow-xs'
                    : 'bg-[#F4F4F0] text-gray-700 border-gray-200 hover:border-black'
                }`}
              >
        ...
        <div className="sticky top-20 z-30 bg-white/95 backdrop-blur-md border border-gray-200/80 p-3 sm:p-4 mb-8 shadow-xs flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-[#111111] text-white text-[10px] font-normal uppercase tracking-[0.18em] hover:bg-black transition-colors"
            >
            ...
            <span className="text-xs font-light text-gray-500 tracking-wide">
              Showing <strong className="font-normal text-gray-900">{filteredProducts.length}</strong> styles
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center border border-gray-200 bg-white">
              <button
            ...
            <div className="flex items-center gap-1.5 text-xs">
              <label htmlFor="plp-sort-select" className="font-light text-gray-500 tracking-wide hidden sm:inline">
                Sort By:
              </label>
              <select
                id="plp-sort-select"
                ...
                className="bg-white border border-gray-200 text-xs font-light tracking-wide px-3 py-1.5 text-gray-900 focus:border-black focus:outline-hidden"
              >
          ...
          {(selectedFit !== 'all' || selectedSize !== 'all' || inStockOnly) && (
            <div className="w-full flex flex-wrap items-center gap-2 pt-2 border-t border-gray-100">
              <span className="text-[9px] uppercase tracking-[0.2em] font-light text-gray-400">Active Filters:</span>
              {selectedFit !== 'all' && (
                <span className="inline-flex items-center gap-1 text-[10px] font-light bg-gray-100 px-2 py-0.5 border border-gray-200">
                  Fit: {selectedFit}
              ...
              <button
                ...
                className="text-[10px] underline text-rose-700 font-normal hover:text-black ml-2 tracking-wider uppercase"
              >
                Clear All
              </button>
            </div>
        ...
        {showFilterDrawer && (
          <div className="bg-white p-6 border border-gray-200/80 mb-8 animate-fadeIn space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              
              {/* Fit Filter */}
              <div>
                <label className="text-[10px] font-normal uppercase tracking-[0.2em] text-gray-900 block mb-2">
                  Fit Preference
                </label>
                <div className="flex flex-wrap gap-2">
                  {['all', 'Tailored', 'Relaxed', 'Oversized'].map((fit) => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-light border ${
                        selectedFit === fit
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['all', 'XS', 'S', 'M', 'L', 'XL', 'XXL'].map((sz) => (
                    <button
                      ...
                      className={`w-8 h-8 text-xs font-light border flex items-center justify-center ${
                        selectedSize === sz
                          ? 'bg-black text-white border-black font-normal'
                          : 'bg-white text-gray-700 border-gray-200 hover:border-black'
                      }`}
        ...
        {filteredProducts.length === 0 ? (
          <div className="bg-white p-12 border border-gray-200/80 text-center space-y-3">
            <span className="material-symbols-outlined text-3xl text-gray-400">filter_alt_off</span>
            <h3 className="text-base text-gray-800 font-light">No products match your filter criteria</h3>
            ...
            <button
              ...
              className="mt-2 inline-block bg-black text-white px-5 py-2 text-[10px] font-normal uppercase tracking-[0.18em]"
            >
              Reset Filters
            </button>
          </div>
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  ...
                  className="group bg-white border border-gray-200/80 overflow-hidden flex flex-col justify-between transition-all duration-300 hover:border-black hover:shadow-xs"
                >
                  ...
                  <div className="relative aspect-[3/4] overflow-hidden bg-neutral-100 cursor-pointer"
                    ...
                    {product.badge && (
                      <div className="absolute top-2.5 left-2.5 z-10">
                        <span
                          className={`text-[8px] font-light uppercase tracking-[0.18em] px-2 py-0.5 text-white ${
                            product.badge === 'Sale'
                              ? 'bg-rose-800'
                    ...
                    {product.viewersCountToday && (
                      <div className="absolute bottom-2.5 left-2.5 z-10 bg-white/90 backdrop-blur-xs px-2 py-0.5 text-[8px] font-light text-gray-700 border border-gray-200 hidden sm:flex items-center gap-1">
                    ...
                    >
                      <div className="text-[9px] font-light uppercase tracking-[0.18em] text-gray-500 mb-1 text-center">
                        Quick Add Size
                      </div>
                      <div className="flex justify-center gap-1">
                        {product.variants.map((v) => (
                          <button
                            ...
                            className={`w-7 h-7 text-[9px] font-normal border transition-colors flex items-center justify-center ${
                              v.inStock
                  ...
                  <div className="p-3.5 sm:p-4 flex flex-col justify-between flex-1 bg-white">
                    <div>
                      {/* Brand & Fit info */}
                      <div className="flex justify-between items-center text-[9px] font-light uppercase tracking-[0.18em] text-gray-500 mb-1">
                      ...
                      <h3
                        onClick={() => onSelectProduct(product.id)}
                        className="text-xs sm:text-sm font-normal text-[#111111] hover:text-gray-500 transition-colors line-clamp-1 cursor-pointer tracking-wide"
                      >
                        {product.title}
                      </h3>
                      ...
                      <div className="flex items-center gap-1.5 mt-2">
                        {product.colors.map((c, idx) => (
                          <span
                            key={idx}
                            className="w-2.5 h-2.5 border border-gray-300 inline-block"
                            style={{ backgroundColor: c.hex }}
                            title={c.name}
                          />
                        ))}
                        <span className="text-[9px] text-gray-400 font-light ml-1">
                    ...
                    <div className="flex items-baseline justify-between pt-3 mt-3 border-t border-gray-100">
                      <div className="flex items-baseline gap-2">
                        <span className="text-xs sm:text-sm font-normal text-[#111111]">
                          ₹{product.price}
                        </span>
                        {product.originalPrice && (
                          <span className="text-[11px] text-gray-400 font-light line-through">
                            ₹{product.originalPrice}
                          </span>
                        )}
                      </div>
                      {product.discountPercentage && (
                        <span className="text-[9px] font-normal text-rose-800 bg-rose-50 px-1.5 py-0.5 border border-rose-100">
